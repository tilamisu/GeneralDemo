...
- (NSInteger)numberOfLinesWithConstrainedToWidth:(CGFloat)width fromFont:(UIFont *)font1 lineSpace:(float)lineSpace
{
    ...
    CTFrameRef frameRef = CTFramesetterCreateFrame(framesetter, CFRangeMake(0, [string length]), path, NULL);
    //获取行数
...
//获取需要绘制出文本前X行数的字数
- (NSInteger)numberOfCharactersWithMaxLineCount:(NSInteger)lineCount
                                      ...
                                      lineSpace:(float)lineSpace
{
    ...
    NSInteger numberOfLines = CFArrayGetCount(lines);
    //读取字数
    NSInteger maxLines = MIN(numberOfLines, lineCount);
    NSInteger totalCharacters = 0;
    NSArray *numberLines = (__bridge NSArray *)lines;
    for (NSInteger i = 0; i < maxLines; i++) {
        CTLineRef line = (__bridge CTLineRef)[numberLines objectAtIndex:i];
        CFIndex glyphCount = CTLineGetGlyphCount(line);
        totalCharacters += glyphCount;
    }
    //释放数据
    CGPathRelease(path);
    CFRelease(frameRef);
